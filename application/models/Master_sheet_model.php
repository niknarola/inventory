...
class Master_sheet_model extends CI_Model
{
	...
	function getRows($params = array(), $searchBy = 'part') {
        ...
        if ($searchBy == "part" || $searchBy == "name") {
            if (!empty($params['search']['keywords'])) {
                $this->db->select('p.id as pid,p.part as part, p.name as product_name, p.description as product_desc,p.release_date as release_date, p.category as category,p.added_as_temp,ps.*,p.is_flagged as is_flagged,pal.pallet_id as pallet,oc.name as original_condition', FALSE);
