...
<div class="row">
	<div class="col-md-12">
		<form method="post" action="<?= $admin_prefix; ?>testing/all_in_one" id="all-in-one" enctype="multipart/form-data">
								...
								<div class="form-group">
									...
									<select name="category1" class="category1 form-control" onchange="get_sub_categories(this.value, 'category2')">
										<?php foreach ($categories as $key => $value) { ?>
											<option <?php echo ($key == 2) ? 'selected' : ''; ?> value="<?= $key ?>"><?= $value ?></option>
										<?php } ?>
									</select>
								</div>
								</div>
...
<script type="text/javascript">
	jQuery(document).ready(function($) {
	    ...
	    get_sub_categories(2, 'category2');
		...
		if(serial!=''){
			...
			.done(function(response) {
				if(response.status==0){
					...
					$('.category1').val(2).trigger('change');
					get_sub_categories(2, 'category2');
